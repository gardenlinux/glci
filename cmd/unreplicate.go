package main

import (
	"context"
	"fmt"

	"github.com/spf13/cobra"
	"github.com/spf13/viper"

	"github.com/gardenlinux/glci/internal/cli"
	"github.com/gardenlinux/glci/internal/glci"
	"github.com/gardenlinux/glci/internal/log"
)

func unreplicateCmd() *cobra.Command {
	c := &cobra.Command{
		Use:   "unreplicate",
		Short: "'Unreplicate Garden Linux release artifacts from artifact sources",
		Args:  cobra.NoArgs,
		RunE:  cli.RunFunc(unreplicate),
	}

	c.Flags().StringP("version", "v", "", "release version")
	c.Flags().StringP("commit", "c", "", "release commit(ish)")
	c.Flags().Bool("steamroll", false, "ignore errors while destroying things")

	return c
}

func unreplicate(ctx context.Context, cfg *viper.Viper, _ []string) error {
	log.Info(ctx, "GLCI", "version", version)

	g, err := glci.New(cfg.AllSettings())
	if err != nil {
		return err
	}

	var stop func() error
	stop, err = g.Start(ctx, g.Publisher.SourceConfigurables()...)
	if err != nil {
		return fmt.Errorf("cannot start artifact sources: %w", err)
	}
	defer func() {
		_ = stop()
	}()

	err = g.Publisher.Unreplicate(ctx, cfg.GetString("version"), cfg.GetString("commit"), cfg.GetBool("steamroll"))
	if err != nil {
		return err
	}

	return stop()
}
